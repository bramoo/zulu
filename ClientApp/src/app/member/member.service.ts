...
export class MemberService {
  ...
  public getMember(id: number): Observable<Member> {
    return this.http.get(this.baseurl + 'api/v1/members/' + id.toString())
      .map(response => response.json() as Member);
  }

  public createMember(member: Member): Observable<boolean> {
    return this.http.post(this.baseurl + 'api/v1/members', member)
      .map(response => response.ok);
  }

  public editMember(member: Member): Observable<boolean> {
    return this.http.put(this.baseurl + 'api/v1/members/' + member.id.toString(), member)
      .map(response => response.ok);
  }

  public createReport(id: number, report: Report) {
    return this.http.post(this.baseurl + "api/v1/members/" + id + "/reports", report)
      .map(response => response.ok);
  }

  public deleteMember(id: number): Observable<boolean> {
    return this.http.delete(this.baseurl + 'api/v1/members/' + id.toString())
      .map(response => response.ok);
  }
}
